/**
 * Editorial report classifiers and builders.
 * Shared by Node report scripts and the SvelteKit app (TS re-export).
 */
// @ts-nocheck
import { existsSync, readFileSync } from 'node:fs';
import { join } from 'node:path';
import { thumbnailPathForAsset } from './thumbnail-path.mjs';
import { createScriptContext } from './report-cli.mjs';
...
export function shotCompletenessFlags(shot) {
	...
	const cameraMissing =
		!shot.camera ||
		(!shot.camera.movement && !shot.camera.movementDescription?.trim());
...
/** @param {import('../../src/lib/types/script.ts').Shot} shot @param {import('../../src/lib/types/script.ts').Take | undefined} take */
