...
import { RULES, scanWithRules } from '$project-tools/lib/trailer-spoilers.mjs';
import { TRAILER_MASTER_OMITTED_FACT_IDS } from '$project-tools/lib/trailer-master-omitted-facts.mjs';
...
describe('master-derived trailer spoiler gate', () => {
	...
	it('every trailer cue sourced from a fact-tagged Festival-master cue has an inherited binding applied', () => {
		...
		const script = getScript(scriptId);
		const festivalPath = join(repositoryRoot, 'data/scripts/light-delay-festival-master.json');
