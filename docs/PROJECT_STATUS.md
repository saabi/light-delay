...
- **Llegada, orden y carrera verificables:** Harlan llega al puente por coincidencia después de una tarea legítima y sólo reacciona ante «la verdadera firma apunta a—». Voss sopesa la implicación de Sorell, la envía a verificar y luego exige que busque a Harlan y no entre sola; la demora resultante y las rutas servicio/central explican quién llega primero.
- **Cadena probatoria y cierre sincronizados:** main, Festival y largo conservan token personal de Zao, snapshot del manifiesto firmado por hardware, auditoría independiente de Elin, contención física de Okoye con Voss en el puente y envío final del informe terrestre sin respuesta. Festival ya implementa E–G en cues, aunque todavía no en tomas.
...
- **Deuda visual del animatic:** inventario provisional de 179 candidatos `needs_regeneration` (112 main + 38 Festival + 29 tráiler) y 12 placeholders main `needs_replacement`. La producción visual queda bloqueada por guion y cobertura de tomas; informes en `npm run report:editorial`.
...
- **Multi-script / Festival Cut (ADR-0001 Accepted):** festival tiene 7 escenas, guion causal A–G y **38 tomas** en A–D; E–G aún no tienen shot list. Las tres tomas nuevas cubren deliberación de Voss y carrera por rutas distintas sin generar medios.
...
2. **Festival Cut.** Datos con 7 escenas, guion A–G y **38 tomas** A–D. Faltan el shot list E–G, imágenes canónicas y el array `sequences` sigue vacío. Ver escaleta y `docs/CUIDADOS_NARRATIVOS.md`.
   Las tomas A–D suman 3:38,2; para respetar un máximo de 5:00, E–G disponen de 1:21,8 o el montaje previo debe recuperar tiempo mediante solapamiento/compresión editorial.
...
5. **Cobertura de Festival y largo.** El largo tiene tratamiento pero todavía no guion dialogado ni animatic; Festival cubre el guion A–G, pero E–G carece de `shotIds`. La cobertura visual debe completarse sólo después de diseñar y aprobar esas tomas.
