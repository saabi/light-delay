...
- **Ledger causal ejecutable:** la cadena de 11 pasos y 13 acciones del corto está en `data/continuity/`; valida hechos previos, conocimiento por actor y referencias a escaleta/tomas. Voss envía la búsqueda por la demora de Sorell y la negación de Harlan, no por conocer aún la muerte. Festival, tráiler y largo declaran `incomplete` sin falso verde. `report:causal-validity` corre en CI.
- **Notas humanas:** contrato ampliado y `docs/PENDING_AUTHOR_NOTES.md` generado desde los JSON (`notes:build` / `notes:check`); el informe actual reúne 151 notas abiertas por prioridad y ruta exacta.
