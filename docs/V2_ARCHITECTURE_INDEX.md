...
The application compatibility gate runs the complete unit suite and browser suite by default. It skips only the exact stale assertion that expects 94 Festival-master shots while current project data contains 85, and the exact Okoye voice-copy browser assertion, under an explicit compatibility environment flag. Dated production history records the shot count changing 94→90→85; the project-data audit reruns both assertions and requires those exact current failure symptoms. `validate:schemas` also reports and quarantines only the exact missing-`inputDigest` error in the newly added 077 singleton result. All other schema errors fail normally.
...
Verification on the integrated line is recorded in GitHub Actions for the final baseline SHA after completion. Previous CI runs established the Studio/core and Pages build gates; the latest project-data run exposed the stale expected-count signature and is being rerun against the exact current 94-versus-85 mismatch. No Pages or Linode deployment occurred. Protocol-2 helper installation remains pending.
