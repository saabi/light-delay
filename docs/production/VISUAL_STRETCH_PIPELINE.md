...
- Ready handoff (no preview flag) requires plan `compiledPrompt`, `runnable: true`, stretch `videoPromptFreeze.status: approved`, and Seedance `executable: true`. It writes `status: ready` / `nonExecutable: false`. Human confirmation is still required before MCP submit.
- **Package-prep report:** when a ready package is presented for go/no-go, report **credits and the full input asset list** (each `references[]` entry: `assetId`, role, path, ledger reuse vs new upload) — not cost alone. Same rule for singleton Seedance packages. Detail: `HIGGSFIELD_MCP.md` §8b step 6.
...
**Movie mode (animatic player):** Registered Seedance clips **replace member stills** while Movie mode plays. After job-level video register, the player builds **playback spans** from the generation plan’s video jobs + `assets.json`. A playable video (`needs_review` or `current`, file on disk) collapses that job’s `memberInputs` into one unmuted Seedance clip (duration = asset `durationMs`). Cue WAVs for those member shots are suppressed so Seedance audio is not doubled. Selected-take `videoAssetId`s also play as singleton spans when no stretch job covers that shot (Festival-master title sting, 039, 040b). Stretch job register still does **not** write `take.videoAssetId`; OTIO take bind remains editorial.

`npm run production:plans` rebuilds stretch jobs from the script. It **re-applies** registered video `outputs.assetId` from `assets.json` (`metadata.stretchJobId` → job id). Do not treat a freshly rebuilt plan with empty video outputs as “no clip exists” if the asset is already registered — rebuild again, or check `applyRegisteredStretchVideoOutputs`. An MP4 under `static/` that was never registered does not play.

Landing a clip without register (stretch) or without `take.videoAssetId` (singleton) leaves the animatic on the storyboard still. Procedure: `AGENT_GENERATION_BRIEF.md` §8.5 / `HIGGSFIELD_MCP.md` §8b.
