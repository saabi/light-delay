...
**Review changes** invokes the deterministic Draft-diff proposal source. The review panel describes screenplay operations in author-facing terms: text revisions show Before/After wording, insertions and removals identify the screenplay element, and movements explain the new placement. The Reject and visually primary Accept actions are explicit. Reject preserves the Draft but changes no authoritative projection or history. Accept records content authorship and deterministic proposal provenance separately from the trusted accepting human, appends a ChangeSet/ProjectRevision, and refreshes the authoritative screenplay.

Re-saving an existing Draft persists its current provisional content without changing the Draft's original project/document base. When authoritative screenplay work advances meanwhile, Studio warns that the saved Draft started from an earlier state; acceptance performs the actual scoped conflict check. A Proposal can reach only one terminal state, so an accept/reject race produces one winner and an explicit already-resolved result for the other caller.
